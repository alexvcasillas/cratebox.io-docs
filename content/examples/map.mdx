...
The following example shows how to create a simple todo list of top-level type `Map<string, Todo>` to keep some task data within it, make use of views and actions to dispatch changes to the state.
 ...
 */
const todoState: TodoState = new Map<string, Todo>();
...
const todoViews = (state: TodoState): TodoViews => ({
  done: Array.from(state.entries())
    .filter(([key, todo]) => todo.done)
    .map(([key, todo]) => todo),
});
